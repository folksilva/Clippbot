...
class EmailHandler(InboundMailHandler):
	def receive(self, message):
		...
		item_key = re.search('suggestion\..*@',message.to).group()[11:][:-1]
		...
		if not item or not isinstance(item,Item):
			...
			logging.error(u"O e-mail de sugestão categoria recebido não é destinado a um item conhecido")
			return
		...
		if sender == None:
			...
			logging.error(u"Quem enviou o e-mail de sugestão categoria para o item %s não é conhecido" % item.key().name())
			return
